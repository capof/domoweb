...
class LoaderTask(threading.Thread):
	...
	def loadWidgets(self):
		...
		WidgetOption.objects.all().delete()
		WidgetSensor.objects.all().delete()
		WidgetCommand.objects.all().delete()
					...
					if os.path.isfile(info):
						...
						for wid, widget in widgetset_widgets.items():
							...
							for pid, param in widget['options'].items():
								...
								p = WidgetOption(id=id, widget_id=widget_id, key=pid, name=param['name'], description=param['description'], type=param['type'])
								if 'default' in param:
